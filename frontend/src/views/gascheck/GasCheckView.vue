...
import {
  Loader2, Flame, CheckCircle2, XCircle,
  RefreshCw, Printer, AlertTriangle, ToggleLeft,
...
import { useGasCheckStore } from '@/stores/gasCheck.store'
...
const gasCheckStore = useGasCheckStore()
...
const form = ref({
  ...
  cageSpecified: true,
  cageMaterialCompliant: true,
  cageDoorsLockable: true,
  sans10087CalloutOnDrawing: true,
  combustibleDistanceM: 3.0,
  openingDistanceM: 1.0,
  noFlamesSign: true,
  noSmokingSign: true,
  noCellphonesSign: true,
  noUnauthorizedEntrySign: true,
  fireExtinguisherSpecified: true,
  extinguisherRatingKg: 9,
  pipeMaterialSpecified: true,
  pipeMaterial: '15mm composite pipe (SABS approved)',
  hasUndergroundPiping: false,
  undergroundDepthMm: 500,
  gasLineShownOnDrawing: true,
  installerCertRequired: true,
  workmanshipCertRequired: true,
})

const result = computed(() => gasCheckStore.getCheck(props.projectId))
const isLoading = computed(() => gasCheckStore.isLoading(props.projectId))

const CATEGORY_COLORS = {
  Storage: 'bg-amber-50 border-amber-200 text-amber-700',
  Distances: 'bg-blue-50 border-blue-200 text-blue-700',
  'Safety Signage': 'bg-rose-50 border-rose-200 text-rose-700',
  Piping: 'bg-cyan-50 border-cyan-200 text-cyan-700',
...
async function runCheck() {
  ...
  isSaving.value = true
  try {
    await gasCheckStore.evaluate(props.projectId, form.value)
    toast.success('Gas installation check complete')
  } catch {
    toast.error('Failed to run gas check')
  } finally {
...
function selectProject(projectItem) {
  ...
  router.push({ name: 'gascheck', params: { projectId: projectItem.id } })
}
...
async function loadProject(id) {
  ...
  try {
    ...
    const saved = await gasCheckStore.fetchCheck(id)
    if (saved?.input) Object.assign(form.value, saved.input)
  } catch {
    toast.error('Failed to load gas check context')
  } finally {
  ...
  <div class="view-page h-[calc(100vh-100px)] flex flex-col print:h-auto print:block">
    <div v-if="!projectId" class="flex-1 flex flex-col gap-4 overflow-hidden print:hidden">
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col gap-4 h-full">
        <div class="flex items-center justify-between shrink-0">
          <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.25em] text-primary/70">Gas Installation</p>
            <h2 class="text-lg font-extrabold tracking-tight text-slate-900 uppercase mt-0.5">Select a Project</h2>
            <p class="text-xs text-slate-500 mt-0.5">Choose a project to run SANS 10087-7 gas installation checks.</p>
          </div>
          <Button
    ...
    <main v-else class="relative flex-1 min-h-0 flex flex-col gap-3">
      <div class="hidden print:block mb-6">
        <h1 class="text-xl font-bold">Gas Installation Checklist - SANS 10087-7:2013</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3 shrink-0 print:hidden">
        <div class="flex items-center gap-3 rounded-xl bg-white border border-slate-200 shadow-sm px-4 py-3">
          <div class="h-8 w-8 rounded-md bg-amber-50 flex items-center justify-center shrink-0">
            <Flame class="h-4 w-4 text-amber-600" />
          </div>
          <div>
            <p class="text-xs text-slate-500">Gas Status</p>
            <p class="text-sm font-bold text-slate-900 leading-tight">{{ form.hasGasInstallation ? 'LPG Installed' : 'No Gas' }}</p>
          </div>
        </div>
      ...
      <div class="flex-1 min-h-0 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col overflow-hidden">
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100 shrink-0 print:hidden">
          <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-primary/70">Gas Installation</p>
            <p class="text-xs text-slate-500 mt-0.5">{{ project?.name ?? 'Project' }} · SANS 10087-7:2013</p>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4">
            <section class="space-y-4 print:hidden">
              <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center gap-3">
                  <Switch id="hasGas" v-model="form.hasGasInstallation" />
                  <div>
                    <Label for="hasGas" class="text-sm font-semibold text-slate-800 cursor-pointer">Project has LPG gas installation</Label>
                    <p class="text-xs text-slate-500">Toggle off if this project has no gas - the check will auto-pass as N/A</p>
                  </div>
                </div>
              </div>

              <template v-if="form.hasGasInstallation">
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Storage Cage</h2>
                  <div class="space-y-3">
                    <div v-for="(item, key) in {
                      cageSpecified: { label: 'Storage cage specified and detailed', clause: 'SANS 10087-7 cl.5.2' },
                      cageMaterialCompliant: { label: '25x25mm sq tubing + expanded steel mesh', clause: 'SANS 10087-7 cl.5.2.1' },
                      cageDoorsLockable: { label: 'Lockable steel front doors', clause: 'SANS 10087-7 cl.5.2.2' },
                      sans10087CalloutOnDrawing: { label: 'SANS 10087 reference on drawing', clause: 'SANS 10087-7' },
                    }" :key="key" class="flex items-start gap-3">
                      <Switch :id="'cage-'+key" v-model="form[key]" class="mt-0.5" />
                      <div>
                        <Label :for="'cage-'+key" class="text-sm text-slate-700 cursor-pointer">{{ item.label }}</Label>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Safety Distances</h2>
                  <div class="grid grid-cols-2 gap-3">
                    <div>
                      <Label class="text-xs font-medium text-slate-600">From combustibles (m)</Label>
                      <Input v-model.number="form.combustibleDistanceM" type="number" step="0.5" class="mt-1 h-9" />
                      <p class="text-xs mt-0.5" :class="form.combustibleDistanceM >= 3.0 ? 'text-emerald-600' : 'text-rose-600'">
                        Min 3m required
                      </p>
                    </div>
                    <div>
                      <Label class="text-xs font-medium text-slate-600">From openings (m)</Label>
                      <Input v-model.number="form.openingDistanceM" type="number" step="0.5" class="mt-1 h-9" />
                      <p class="text-xs mt-0.5" :class="form.openingDistanceM >= 1.0 ? 'text-emerald-600' : 'text-rose-600'">
                        Min 1m required
                      </p>
                    </div>
                  </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Safety Signage & Fire</h2>
                  <div class="space-y-3">
                    <div v-for="(item, key) in {
                      noFlamesSign: { label: 'No Open Flames sign', clause: 'SABS 1186' },
                      noSmokingSign: { label: 'No Smoking sign', clause: 'SABS 1186' },
                      noCellphonesSign: { label: 'No Cellphones sign', clause: 'SANS 10087-7 cl.6.1' },
                      noUnauthorizedEntrySign: { label: 'No Unauthorized Entry sign', clause: 'SABS 1186' },
                      fireExtinguisherSpecified: { label: 'Fire extinguisher location shown', clause: 'SANS 10087-7 cl.6.2' },
                    }" :key="key" class="flex items-start gap-3">
                      <Switch :id="'sig-'+key" v-model="form[key]" class="mt-0.5" />
                      <div>
                        <Label :for="'sig-'+key" class="text-sm text-slate-700 cursor-pointer">{{ item.label }}</Label>
                        <p class="text-xs text-slate-400">{{ item.clause }}</p>
                      </div>
                    </div>
                    <div v-if="form.fireExtinguisherSpecified">
                      <Label class="text-xs font-medium text-slate-600">Extinguisher rating (kg)</Label>
                      <Input v-model.number="form.extinguisherRatingKg" type="number" class="mt-1 h-9" />
                      <p class="text-xs mt-0.5" :class="form.extinguisherRatingKg >= 9 ? 'text-emerald-600' : 'text-rose-600'">
                        Min 9kg dry powder
                      </p>
                    </div>
                  </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Piping</h2>
                  <div class="space-y-4">
                    <div class="flex items-start gap-3">
                      <Switch id="pipe-shown" v-model="form.gasLineShownOnDrawing" class="mt-0.5" />
                      <div>
                        <Label for="pipe-shown" class="text-sm text-slate-700 cursor-pointer">Gas line route shown on layout plan</Label>
                        <p class="text-xs text-slate-400">SANS 10087-7 cl.4.1</p>
                      </div>
                    </div>
                    <div class="flex items-start gap-3">
                      <Switch id="pipe-spec" v-model="form.pipeMaterialSpecified" class="mt-0.5" />
                      <div>
                        <Label for="pipe-spec" class="text-sm text-slate-700 cursor-pointer">Pipe material specified</Label>
                      </div>
                    </div>
                    <div v-if="form.pipeMaterialSpecified">
                      <Label class="text-xs font-medium text-slate-600">Pipe specification</Label>
                      <Input v-model="form.pipeMaterial" class="mt-1 h-9" placeholder="e.g. 15mm composite pipe" />
                    </div>
                    <div class="flex items-start gap-3">
                      <Switch id="underground" v-model="form.hasUndergroundPiping" class="mt-0.5" />
                      <div>
                        <Label for="underground" class="text-sm text-slate-700 cursor-pointer">Has underground piping</Label>
                      </div>
                    </div>
                    <div v-if="form.hasUndergroundPiping">
                      <Label class="text-xs font-medium text-slate-600">Underground depth (mm)</Label>
                      <Input v-model.number="form.undergroundDepthMm" type="number" class="mt-1 h-9" />
                      <p class="text-xs mt-0.5" :class="form.undergroundDepthMm >= 500 ? 'text-emerald-600' : 'text-rose-600'">
                        Min 500mm
                      </p>
                    </div>
                  </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Certification Callouts</h2>
                  <div class="space-y-3">
                    <div v-for="(item, key) in {
                      installerCertRequired: { label: 'Approved LPG installer required (noted)', clause: 'Gas Act 85 of 2001' },
                      workmanshipCertRequired: { label: 'Certificate of workmanship required (noted)', clause: 'SANS 10087-7 cl.8.2' },
                    }" :key="key" class="flex items-start gap-3">
                      <Switch :id="'cert-'+key" v-model="form[key]" class="mt-0.5" />
                      <div>
                        <Label :for="'cert-'+key" class="text-sm text-slate-700 cursor-pointer">{{ item.label }}</Label>
                        <p class="text-xs text-slate-400">{{ item.clause }}</p>
                      </div>
                    </div>
                  </div>
            ...
            <section class="space-y-4 min-w-0">
              ...
              <div v-else-if="!result" class="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center">
                <Flame class="h-10 w-10 mx-auto text-slate-300 mb-3" />
              ...
              <template v-else>
                <div v-if="!result.hasGasInstallation"
                  class="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center">
                  <ToggleLeft class="h-10 w-10 mx-auto text-slate-400 mb-3" />
                  <p class="text-sm font-semibold text-slate-700">No Gas Installation</p>
                  <p class="text-xs text-slate-500 mt-1">This project has been marked as having no LPG gas installation. SANS 10087-7 does not apply.</p>
                </div>
                ...
                <template v-else>
                  <div class="rounded-xl border p-5 shadow-sm" :class="result.overallPass ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'">
                    <div class="flex items-center gap-4">
                      ...
                      <div class="flex-1">
                        <p class="text-base font-bold" :class="result.overallPass ? 'text-emerald-800' : 'text-rose-800'">
                          {{ result.overallPass ? 'All SANS 10087-7 Requirements Met' : 'Gas Compliance Issues Found' }}
                        </p>
                        <p class="text-sm mt-0.5" :class="result.overallPass ? 'text-emerald-600' : 'text-rose-600'">
                          {{ passCount }} of {{ totalCount }} items compliant
                    ...
                    <p class="mt-2 text-xs text-slate-500">
                      {{ result.results?.sansReference }} · {{ new Date(result.checkedAt).toLocaleString('en-ZA') }}
                    </p>
                  </div>
                  ...
                  <div class="rounded-lg bg-amber-50 border border-amber-200 p-4 flex gap-3">
                    ...
                    <p class="text-xs text-amber-800">
                      <strong>Gas Act 85 of 2001:</strong> All LPG installations must be performed by a registered installer. A certificate of compliance must be provided before occupation. This checklist verifies drawing callouts only - it does not replace the installer's physical inspection and certification.
                    </p>
                  </div>
